...
export function ProcessEditor({ processId, onBack }: ProcessEditorProps) {
  const { addProcess, updateProcess, processes, steps: contextSteps, branches: contextBranches } = useProcessContext();
  ...
  useEffect(() => {
    if (processId) {
      ...
      if (process) {
        ...
        setProcessCategory(process.category);
        setCreatedAtDate(process.createdAt);
        
        const existingSteps = contextSteps
          .filter(s => s.processId === processId)
          .sort((a, b) => a.stepNumber - b.stepNumber);
        
        setSteps(existingSteps.map(step => {
          const branches = contextBranches.filter(b => b.stepId === step.id);
          ...
          return {
            ...
            yesNextStep: yesBranch?.nextStepId,
            noDetails: noBranch?.description,
            noNextStep: noBranch?.nextStepId,
          };
        }));
        setEditorMode('edit-mode-select');
      }
    }
  }, [processId, processes, contextSteps, contextBranches]);
  ...
  const handleSave = async () => {
    ...
    const processData: Process = {
      ...
      createdAt: processId ? createdAtDate : currentDate,
      updatedAt: currentDate,
    };

    // Create ProcessStep objects (will be updated with actual process ID after creation)
      ...
      if (step.isDecision) {
        ...
        if (step.yesDetails || step.yesNextStep) {
          processBranches.push({
            ...
            stepId: stepId,
            condition: 'yes',
            nextStepId: step.yesNextStep ? `step-${newProcessId}-${step.yesNextStep}` : null,
        ...
        if (step.noDetails || step.noNextStep) {
          processBranches.push({
            ...
            stepId: stepId,
            condition: 'no',
            nextStepId: step.noNextStep ? `step-${newProcessId}-${step.noNextStep}` : null,
    ...
    try {
      // Save to context (which will sync with database)
      if (processId) {
        const success = await updateProcess(processId, processData, processSteps, processBranches);
        if (success) {
          alert('Process updated successfully! Changes are now permanently stored.');
        }
      } else {
        const actualProcessId = await addProcess(processData, processSteps, processBranches);
        if (actualProcessId) {
          alert(`Process created successfully with ID: ${actualProcessId}! It is now permanently stored in the system.`);
        }
      }
      ...
      alert(`Error saving process: ${(error as Error).message || 'Please try again.'}`);
    } finally {
      setIsSaving(false);
    }
  };
                      ...
                      <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={step.yesNextStep || ''}
                              onChange={(e) => updateStep(index, 'yesNextStep', e.target.value)}
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                              placeholder="e.g., 2 or Step 2"
                            />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={step.noNextStep || ''}
                              onChange={(e) => updateStep(index, 'noNextStep', e.target.value)}
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                              placeholder="e.g., 7 or End process"
                            />
                          </div>
                        </div>
                      </div>
