      ...
      if (processId) {
        ...
        if (result && result.steps.length > 0) {
          ...
          const formSteps: StepForm[] = existingSteps.map((step) => {
            ...
            const nextStepNumber = step.nextStepId === '__END__'
              ? null
  ...
  const handleSave = () => {
    ...
    const processSteps: ProcessStep[] = steps.map(step => {
      ...
      if (!step.isDecision) {
        if (step.nextStepNumber === null) {
          ...
          nextStepId = '__END__';
        } else if (step.nextStepNumber !== undefined) {
          ...
          // Persist default sequential flow explicitly for API-backed storage.
          const nextSequentialStep = steps.find(s => s.stepNumber === step.stepNumber + 1);
          if (nextSequentialStep) {
            nextStepId = nextSequentialStep.id.startsWith('new-')
              ? `step-${newProcessId}-${nextSequentialStep.stepNumber}`
              : nextSequentialStep.id;
          } else {
            nextStepId = null;
          }
        }
    ...
    steps.forEach((step, index) => {
      if (step.isDecision) {
        ...
        const hasYesBranch = step.yesNextStep !== undefined || !!step.yesDetails?.trim();
        if (hasYesBranch) {
          ...
          const yesNextStepId = step.yesNextStep === null
            ? '__END__'
            : yesTargetStep
            ? yesTargetStep.id
            : null;
          ...
          processBranches.push({
            ...
            stepId: stepId,
            condition: 'yes',
            nextStepId: yesNextStepId,
            description: step.yesDetails || '',
          });
        }
        
        const hasNoBranch = step.noNextStep !== undefined || !!step.noDetails?.trim();
        if (hasNoBranch) {
          // Parse step number from string to number
          ...
          const noNextStepId = step.noNextStep === null
            ? '__END__'
            : noTargetStep
            ? noTargetStep.id
            : null;
