...
export function AdminDashboard() {
  ...
  const handleDeleteProcess = async (processId: string) => {
    if (confirm('Are you sure you want to delete this process? This action cannot be undone and the process will be permanently removed from the system.')) {
      setIsDeleting(processId);
      try {
        const success = await deleteProcess(processId);
        if (success) {
          alert('Process deleted successfully! It has been permanently removed from the system.');
        }
      } catch (error) {
        console.error('Error deleting process:', error);
        alert(`Error deleting process: ${(error as Error).message || 'Please try again.'}`);
      } finally {
        setIsDeleting(null);
      }
    }
  ...
  if (viewMode === 'edit') {
    return <ProcessEditor processId={editingProcessId} onBack={handleBackToList} />;
  }
