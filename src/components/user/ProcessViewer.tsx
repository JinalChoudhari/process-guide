...
type PathProgress = {
  [pathId: string]: number; // Maps path ID to current step index in that path, -1 END reached, -2 no configured next step
};
...
export function ProcessViewer({ processId }: ProcessViewerProps) {
  ...
  const handleNextStep = (pathId: string, pathSteps: ProcessStep[], currentIndex: number) => {
    ...
    if (nextIndex >= pathSteps.length) {
      ...
      if (currentStep.nextStepId === '__END__') {
        nextStepId = null;
      ...
      if (!nextStepId) {
        setPathProgress(prev => ({
          ...prev,
          [pathId]: -2 // -2 indicates no linked next step configured
        }));
        return;
      }
    }
  ...
  const getCommonTailInfo = (leftPath: ProcessStep[], rightPath: ProcessStep[]) => {
    let i = leftPath.length - 1;
    let j = rightPath.length - 1;
    let commonLength = 0;

    while (i >= 0 && j >= 0 && leftPath[i].id === rightPath[j].id) {
      commonLength += 1;
      i -= 1;
      j -= 1;
    }
    ...
    return {
      leftCommonTail: commonLength > 0 ? leftPath.slice(leftPath.length - commonLength) : [],
      rightCommonTail: commonLength > 0 ? rightPath.slice(rightPath.length - commonLength) : [],
    };
  };
  ...
  const renderStepCard = (
    ...
    const isLastStepInPath = currentIndex === pathSteps.length - 1;
    const showTerminalMessage = Boolean(terminalState && isLastStepInPath);

    return (
      <div
        key={`${pathId}-${step.id}`}
        className="mb-6 animate-fadeIn w-[520px] max-w-none shrink-0"
      >
        <div className={`border-2 ${borderColor} ${bgColor} rounded-lg p-6 shadow-lg transition-all hover:shadow-xl`}>
          ...
          {!isDecision && (
            <div className="mt-6">
              ...
              {showTerminalMessage && terminalState === 'end' && (
                <p className="mt-2 text-xs text-green-700">
                  End process reached.
                </p>
              )}
              {showTerminalMessage && terminalState === 'noLink' && (
                <p className="mt-2 text-xs text-amber-700">
                  No further linked steps.
                </p>
              )}
            </div>
          )}
        ...
        {isDecision && bothPathsVisible && (
          <div className="mt-8">
            {(() => {
              // Compare branch tails using explicit links only.
              const yesSteps = buildPath(yesBranch?.nextStepId, new Set<string>());
              const noSteps = buildPath(noBranch?.nextStepId, new Set<string>());
              const commonTailInfo = getCommonTailInfo(yesSteps, noSteps);
              const mergeStartYesId = commonTailInfo.leftCommonTail.length > 0 ? commonTailInfo.leftCommonTail[0].id : undefined;
              const mergeStartNoId = commonTailInfo.rightCommonTail.length > 0 ? commonTailInfo.rightCommonTail[0].id : undefined;
              const mergeStartId = mergeStartYesId;
              ...
              return (
                <>
                  <div className="overflow-x-auto pb-2">
                    ...
                    {/* YES Path (Left) */}
                    <div className="border-l-4 border-green-500 pl-6 w-[560px] shrink-0">
                      ...
                      {renderPath(`${pathId}-yes`, yesBranch?.nextStepId, 'yes', depth + 1, mergeStartYesId)}
                    </div>

                    {/* NO Path (Right) */}
                    <div className="border-l-4 border-red-500 pl-6 w-[560px] shrink-0">
                      <div className="mb-4 flex items-center gap-2">
                      ...
                      {renderPath(`${pathId}-no`, noBranch?.nextStepId, 'no', depth + 1, mergeStartNoId)}
                    </div>
                    </div>
                  </div>

                  {mergeStartId && (
                    <div className="mt-8 border-t-2 border-dashed border-blue-300 pt-6">
                      <div className="mb-4">
                        <span className="px-4 py-2 bg-blue-600 text-white rounded-md font-bold">
                          MERGED PATH (COMMON STEPS)
                        </span>
                      </div>
                      {renderPath(`${pathId}-merged`, mergeStartId, 'main', depth + 1)}
                    </div>
                  )}
                </>
              );
  ...
  const renderPath = (
    ...
    const fullPathSteps = buildPath(startStepId);
    const stopIndex = stopBeforeStepId
      ? fullPathSteps.findIndex((s) => s.id === stopBeforeStepId)
      : -1;
    const pathSteps = stopIndex >= 0 ? fullPathSteps.slice(0, stopIndex) : fullPathSteps;
    const currentProgress = pathProgress[pathId];

    const terminalState =
      currentProgress === -1 ? 'end' : currentProgress === -2 ? 'noLink' : undefined;
    ...
    return (
      <div className="w-max min-w-max">
        {pathSteps.map((step, index) => {
          const isVisible = currentProgress < 0 ? true : index <= (currentProgress || 0);
          return renderStepCard(step, pathId, index, isVisible, pathType, pathSteps, depth, terminalState);
        })}
      </div>
    );
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div>
        ...
        {/* Process Flow */}
        <div className="bg-gray-50 rounded-lg p-6 min-h-[400px] overflow-x-auto">
